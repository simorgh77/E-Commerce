...
const ProductsCart = ({item}:any) => {
  ...
  console.log(item)
    ...
    return (
        <div className='w-100 product_cart'>
           <Card className='product_cart_main w-100'  style={{
           cursor:'pointer',backgroundColor:"transparent",border:'none'}} 
           onClick={()=>history.push(`/Product_Details${item.id}`)}>

             <div className='w-100'>
  <Card.Img variant="top" className="img-fluid w-100" src={item.image}
  />
  </div>
  <Card.Body className='d-flex w-100 flex-column justify-content-between '>
    <Card.Title style={{fontSize:'100%'}}>{item.name}</Card.Title>
